...
struct TimerBottomSheetView: View {
    ...
    var body: some View {
        GeometryReader { geometry in
            ...
            VStack(spacing: 0) {
                VStack(alignment: .leading, spacing: layout.mainSpacing) {
                    ...
                    // Preset buttons and custom picker
                    VStack(alignment: .leading, spacing: layout.mainSpacing) {
        ...
        .onAppear {
            // Initialize from last saved duration (so Custom shows last saved time when opened)
            let totalSeconds = Int(viewModel.savedTimerDuration)
            if totalSeconds == 2 * 60 {
                ...
                selectedDetent = PresentationDetent.large
            }
    ...
    private func setTimer() {
        ...
        // Save timer duration (does NOT start it)
        viewModel.setTimer(duration: duration)
